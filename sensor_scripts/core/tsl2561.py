...
class TSL2561(object):
  ...
  def run(busnumber=1, tsl_adress=0x39):
    ...
    ambiance_lowbyte = i2cBus.read_byte_data(adressTSL2561, 0x8c)
    ambiance_highbyte = i2cBus.read_byte_data(adressTSL2561, 0x8d)
    ambiance = (ambiance_highbyte * 256) + ambiance_lowbyte

    ir_lowbyte = i2cBus.read_byte_data(adressTSL2561, 0x8e)
    ir_highbyte = i2cBus.read_byte_data(adressTSL2561, 0x8f)
    ...
    moisture = {'sensor': Sensor.get(Sensor.name == 'light'),
                'plant': plant,
                'value': lux}
