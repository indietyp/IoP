...
(function() {
  var change_wizard_general_settings_responsible, delete_general_settings_responsible, get_general_settings_responsibles, get_notification_message_content, get_notification_messages, submit_notification_message;
  ...
  get_notification_message_content = function(uuid) {
    ...
    return request.done(function(msg) {
      msg = JSON.parse(msg);
      return $('.notification_message_content').html(msg);
    });
  };
  ...
  get_general_settings_responsibles = function() {
    ...
    return request.done(function(msg) {
      ...
      for (i = 0, len = msg.length; i < len; i++) {
        person = msg[i];
        tab = '<div class="item"> <div class="right floated content"> <div class="ui small basic icon buttons"> <button class="ui button"> <div class="ui radio checkbox"> <input type="radio" name="wizard" [[checked]] onChange="change_wizard_general_settings_responsible(this, \'[[identifier]]\')"> <label><i class="wizard icon"></i> </label> </div> </button> <button onclick="$(\'.[[identifier]]\').accordion(\'toggle\', 0);" class="ui button"><i class="edit icon"></i> </button> <button class="ui button"><i class="remove icon"></i> </button> </div> </div><img src="https://source.unsplash.com/category/nature" class="ui avatar image"> <div class="content">[[name]]</div> <div class="ui accordion [[identifier]]"> <div style="height:0;padding:0" class="title"></div> <div style="margin-top:1%" class="content"> <div class="ui grid"> <div class="two column row relaxed"> <div style="margin-bottom:1%" class="column"> <div class="ui fluid left icon input"> <input type="text" placeholder="Name" value="[[name]]"><i class="tag icon"></i> </div> </div> <div class="column"> <div class="ui fluid left icon input"> <input type="text" placeholder="EMail" value="[[email]]"><i class="home icon"></i> </div> </div> </div> </div> <div class="ui two buttons"> <div class="ui basic button fluid green" onclick="submit_responsible_changes([[identifier]])">Apply</div> <div class="ui basic button fluid red">Reset</div> </div> </div> </div> <script type="text/javascript"> </script> </div>';
